...
import com.ufrb.lardosidosos.repository.IFichaAdmissaoRepository;
...
public class FichaAdmissaoService implements IFichaAdmissaoService {
	...
	public void verificaSeFichaAdmissaoExiste(Long id) {
		if(!this.fichaAdmissaoRepository.findById(id).isPresent()) throw new NegocioException(NotFoundErrorMsg.NOT_FOUND_FICHA_ADMISSAO.getValor(), HttpStatus.NOT_FOUND);
	}
	...
	public FichaAdmissao buscar(Long id) {
		...
		if (!fichaAdmissaoOptional.isPresent()) {
			throw new NegocioException(NotFoundErrorMsg.NOT_FOUND_FICHA_ADMISSAO.getValor(), HttpStatus.NOT_FOUND);
		}
